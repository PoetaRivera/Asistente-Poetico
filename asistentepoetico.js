...
$(document).ready(function (e) {
    ...
    $("#boton1").click(principal);
    $("#boton2").click(limpiar);
    $("#outtext1").scroll(miScrollUno);
    $("#outtext2").scroll(miScrollDos);
    $("#outtext3").scroll(miScrollTres);
    $("#outtext4").scroll(miScrollCuatro);

});

tipoSina = 2;//determina el tipo de sinalefa
...
// Sincroniza el Scroll entre los textarea de  texto y de número de sílabas.
function miScrollUno(e) {
    evento=e;
    
    let elem1 = document.getElementById("outtext1");
    let elem2 = document.getElementById("outtext2");

    let y = elem1.scrollTop;
    let x = elem1.scrollLeft;
    elem2.scrollTo(x, y);
}

function miScrollDos() {
   
    let elem1 = document.getElementById("outtext1");
    let elem2 = document.getElementById("outtext2");

    let y = elem2.scrollTop;
    let x = elem2.scrollLeft;
    elem1.scrollTo(x, y);
}

function miScrollTres() {
    
    let elem3 = document.getElementById("outtext3");
    let elem4 = document.getElementById("outtext4");

    let y = elem3.scrollTop;
    let x = elem3.scrollLeft;
    elem4.scrollTo(x, y);
}

function miScrollCuatro() {
    let elem3 = document.getElementById("outtext3");
    let elem4 = document.getElementById("outtext4");

    let y = elem4.scrollTop;
    let x = elem4.scrollLeft;
    elem3.scrollTo(x, y);
}


function limpiar() {// limpia todo
...
function determinaAcentoPalabra(pal) {
    ...
    if (numeroSilabas != 1) {
        for (let i = 0; i < numeroSilabas; i++) {//determina si la palabra lleva tilde
        ...
        if (indicadorAcento == 2) { //Si palabra no lleva tilde
            let ultimaSilaba = arregloEnSilabas[(numeroSilabas) - 1];
            let ultimaLetra = ultimaSilaba[ultimaSilaba.length - 1];
            if (ultimaLetra == "s" || ultimaLetra == "n" || ultimaLetra == "a" || ultimaLetra == "e" || ultimaLetra == "i" || ultimaLetra == "o" || ultimaLetra == "u") {
...
function depurarVerso(fila) {
    let arregloCaracteresNormales = ["a", "e", "i", "o", "u", "á", "é", "í", "ó",
        "ú", "h", "ä", "ë", "ï", "ö", "ü", "b", "c", "d", "f", "g", "j", "k", "l", "m",
        "n", "ñ", "p", "q", "r", "s", "t", "v", "w",
        "x", "y", "z", " "];
...
function segundo(filas) {
    ...
    if (numPalabras > 2) {
        for (let i = 1; i < numPalabras - 1; i++) {
            if (((versoConS[i].length == 1) || (versoConS[i].length == 2)) && (sinalefaa[i - 1] && sinalefaa[i])) {
                let cadenaS = versoConS[i - 1] + " " + versoConS[i] + " " + versoConS[i + 1];
                tipoSina = 3;
                let misSilabas = obtenerSilabas(cadenaS);
                if (misSilabas[0] != "0") {
                    trisinalefaa = sinalefaTresPalabras(misSilabas[0], misSilabas[1], misSilabas[2]);
                    let j = (i - 1) / 2;
                    if (trisinalefaa) {
                        versoSalida[i - 1] = versoSalida[i - 1].concat("~");
                        versoSalida[i] = versoSalida[i].concat("~");
                    } else {// salida de hay sinalefa tres palabras
                        versoSalida[i] = versoSalida[i].concat("~");
                    }
                } else {//salida imposible sinalefa de tres palabras

                    versoSalida[i] = versoSalida[i].concat("~");

                }

            } else {//salida condiciones posible sinalefa tres palabras
                if (sinalefaa[i - 1]) {
                    versoSalida[i - 1] = versoSalida[i - 1].concat("~");
    ...
    } else {//salida numero de palabras mayor de dos
        if (numPalabras == 2) {
            if (sinalefaa == true) {
...
function contarSilabasPoetico(v) {
    ...
    total = -conta + determinaAcentoPalabra(verso[verso.length - 1]);
...
function obtenerSilabas(palabras) {// extrae silabas casos dos y tres palabras. Para luego determinar sinalefa
    // let versoEntrada = $("#intext1").val();
    versoEntrada = palabras;
    let versoSalida = [];
    versoSalida = leerVerso(versoEntrada);
    if (tipoSina == 2) { //caso dos palabras
        ...
        return misSilabas;
    } else if (tipoSina == 3) {//caso tres palabras
        let palabra1 = versoSalida[0];
        let palabra2 = versoSalida[1];
...
function sinalefaTresPalabras(silaba1, silaba2, silaba3) {
    ...
    let vocalesCompleto = vocalesPalUno.concat(vocalesPalDos).concat(vocalesPalTres);
...
function sinalefaDosPalabras(silaba1, silaba2) {
    ...
    largoVocalesUno = vocalesPalUno.length;
    largoVocalesDos = vocalesPalDos.length;
    ...
    for (let i = largoPalabra; i >= 0; i--) {
        ...
        if (palabra[i] == "y") {
            if (largoPalabra == 1) {
                ...
                } else {//caso palabra no invertida
                    for (let j = 0; j < vocales.length; j++) {
                        if (palabra[i - 1] == vocales[j]) {
                            palabraCopia[i] = "i";
                        }
...
function separaPalabra(mipalabra) {
    ...
    i = 0;
    while (i < cadena.length) {//examina caracter por caracter de la palabra
        ...
        if (esvocal == false) {
            ...
            if (esr && i != 0) { //si el caracter actual es r, verifica si el caracter anterior es especial
                j = 0;
                for (j = 0; j < largo_r; j++) {
            ...
            if (esl && i != 0) {//si el caracter actual es l, verifica si el caracter anterior es especial
                j = 0;
                for (j = 0; j < largo_l; j++) {
            ...
            if (esh && i != 0) {//si el caracter actual es h, verifica si el caracter anterior es especial o vocal
                j = 0;
                for (j = 0; j < largo_h; j++) {
...
function triptongoSilaba(a) {
    ...
    let vocales = ["a", "e", "i", "o", "u", "á", "é", "í", "ó", "ú"];
    let consonantes = ["h", "b", "c", "d", "f", "g", "j", "k", "l", "m", "n", "ñ", "p", "q", "r", "s", "t", "v", "w", "x", "y", "z"];
    let especiales = ["h"];
...
function hiatoSilaba(a) {
    ...
    let vocales = ["a", "e", "i", "o", "u", "á", "é", "í", "ó", "ú"];
    //String pdt[] = {"", "", "", "", "", "", "", "", "", ""};
    let consonantes = ["h", "b", "c", "d", "f", "g", "j", "k", "l", "m", "n", "ñ", "p", "q", "r", "s", "t", "v", "w", "x", "y", "z", "r", "l"];
    let especiales = ["h"];
...
function hiato(s) {
    ...
    b = dip == ("aa") || dip == ("ee") || dip == ("oo") || dip == ("ii") || dip == ("uu") || dip == ("ae") || dip == ("ea") || dip == ("ao") || dip == ("oa") || dip == ("eo") || dip == ("oe") || dip == ("aí") || dip == ("aú") || dip == ("eí") || dip == ("eú") || dip == ("oí") || dip == ("oú") || dip == ("ía") || dip == ("úa") || dip == ("íe") || dip == ("úe") || dip == ("ío") || dip == ("úe");
...
function cuatroSilaba(a) {
    ...
    let vocales = ["a", "e", "i", "o", "u", "á", "é", "í", "ó", "ú"];
    let consonantes = ["h", "b", "c", "d", "f", "g", "j", "k", "l", "m", "n", "ñ", "p", "q", "r", "s", "t",
        "v", "w", "x", "y", "z", "r", "l"];
    let especiales = ["h"];
    let silaba = "";
    let secuencia = "";
...
function sinalefa(v) {
    ...
    // Fuerte debil
    let caso1 = ["ao", "ae", "au", "ai", "ay", "oe", "ou", "oi", "oy", "eu", "ei", "ey", "ui", "uy", "áo",
        "áe", "áu", "ái", "áy", "óe", "óu", "ói", "óy", "éu", "éi", "éy", "úi", "úy"];
    // Debil fuerte
    let caso2 = ["ia", "ya", "ie", "ye", "io", "yo", "iu", "yu", "ua", "ue", "uo", "ea", "eo", "oa", "iá",
        "yá", "ié", "yé", "ió", "yó", "iú", "yú", "uá", "ué", "uó", "eá", "eó", "oá"];
    // Vocales iguales
    let caso3 = ["aa", "ee", "ii", "yy", "oo", "uu", "aá", "eé", "ií", "oó", "uú", "áa", "ée", "íi", "óo",
        "úu"];

    let caso4 = ["ía", "íe", "ío", "íu", "íy", "úá", "úé", "úó", "úy", "éa", "éo", "éy", "óa", "óy", "aí",
        "eí", "oí", "uí", "yí", "aú", "eú", "oú", "yú", "aé", "oé", "yé", "aó", "yó"];
